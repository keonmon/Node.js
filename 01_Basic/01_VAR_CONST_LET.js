...
// 값을 변경할 수 있는 변수를 사용하려면, 앞에서 사용한 var변수나, 서버프로그래밍에서 주로 사용하는 'let 변수'를 사용한다.
// var과 let변수의 차이는 다소 많지는 않으며, 명백히 다른 점이 있지만 현재 사용상 구분할 일이 그렇게 많지 않다.
// 버전업과 제작자 사이에서 있을 수 있는 차이와 그의 차이점은 아래에 설명한다.
let b1 = 0;
b1 = 1;     //정상 실행됨.
...
// 1. 변수 선언에는 기본적으로 const를 사용하고, 재할당이 필요한 경우에 한해 let을 사용하는 것을 권장한다.
//      - 객체를 재할당하는 경우는 생각보다 흔하지 않으므로, 객체 변수 또한 const를 사용하는 것을 권장.
//      -  const를 사용하면 의도치 않은 재할당을 방지해 주기 때문에 데이터의 안전을 보장받을 수 있다.
